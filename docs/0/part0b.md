...
Coding in the recommended style begins in [part 1](/1/part1.html).
...
Prior to the AJAX era, all web pages worked like the [traditional web application](/part0b.html#traditional-web-applications) we saw earlier in this chapter.
...
Nowadays URLs like these would not be considered acceptable, as they don't follow the generally acknowledged conventions of [RESTful](https://en.wikipedia.org/wiki/Representational_state_transfer#Applied_to_Web_services) APIs, which we'll look into more in [part 3](/3/part3.html)
...
The Notes page gives some of the responsibility, generating the HTML code for existing notes, to the browser. The browser tackles this task by executing the JavaScript code it fetched from the server. The code fetches the notes from the server as JSON-data and adds HTML elements for displaying the notes to the page using the [DOM-API](/part0b.html#document-object-model-or-dom).
...
During this course, we will get familiar with [VueJS](https://v3.vuejs.org/) and the [Vuex](https://vuex.vuejs.org/) library, which are frequently used together.
...
All necessary information for doing this, and the next two exercises, can be found from the text of [this part](/part0b.html#forms-and-http-post).
